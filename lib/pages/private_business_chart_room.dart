...
class PrivatedBusinessChatRoom extends StatefulWidget {
  const PrivatedBusinessChatRoom({super.key, required this.roomId});

  final int roomId;

  @override
  State<PrivatedBusinessChatRoom> createState() =>
      _PrivatedBusinessChatRoomState();
}

class _PrivatedBusinessChatRoomState extends State<PrivatedBusinessChatRoom> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: theAppBar(context, "Bussiness room ${widget.roomId}"),
            ...
            GraphQLConsumer(
              builder: (GraphQLClient client) {
                final options = WatchQueryOptions(
                  document: gql(r"""
                  query($roomId:Int!){
                  bussinessRoomMessages(roomId:$roomId){
                    id
                    content
                    sender{
                      username
                ...
                return StreamBuilder<QueryResult>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final comments =
                        (snapshot.data!.data?['bussinessRoomMessages']
                                as List<dynamic>?) ??
                            [];
                          ...
                          onPressed: () async {
                            ...
                            MutationOptions options = MutationOptions(
                              document: gql(r"""
                                     mutation($roomId:Int!, $content:String!){
                                        createBussinessMessage(roomId: $roomId, content:$content){
                                        successMessage
                                      }
                                     }
                                    """),
                            ...
                            if (result.hasException) {
                              ...
                              final successMessage =
                                  result.data?['createBussinessMessage']
                                      ?['successMessage'];
                              showDialog(
                                context: context,
